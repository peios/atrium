...
while True:
    ...
    if t == 'logon_start':
        ...
        if any(x['credential_ref']==7 and x['data']=='secret' for x in r['answers']):
            ...
            env = {'USER': users[r['conv']], 'ATRIUM_DISPLAY_NAME': 'Jack P', 'ATRIUM_SESSION': str(sid), 'ATRIUM_APPS_DIR': os.environ.get('ATRIUM_APPS_DIR',''), 'HOME': '/', 'PATH': os.environ.get('PATH', '/usr/bin'), 'LD_LIBRARY_PATH': os.environ.get('LD_LIBRARY_PATH',''), **{k: v for k, v in os.environ.items() if k.startswith('FAKE_')}}
